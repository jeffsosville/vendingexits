...
<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  console.log(`   - sitemap-cities.xml (${Object.values(citiesByState).reduce((sum, cities) => sum + cities.size, 0)} pages)`);
  console.log(`   - sitemap-listings.xml (${listings.length} pages)`);
  console.log(`   Total: ${12 + uniqueStates.length + Object.values(citiesByState).reduce((sum, cities) => sum + cities.size, 0) + listings.length} pages`);
};
